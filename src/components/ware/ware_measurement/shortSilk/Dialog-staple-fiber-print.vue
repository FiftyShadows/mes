...
<template>
  <div class="package-print" ref="printBox">
    <ul>
      <li v-for="item in printData" :key="item.kindName">
        <div class="top-box">
          <div class="left-box">
            <div class="line">{{item.productType}}</div>
            <div class="line line-indent">{{item.silkSpec}}</div>
            <div class="line">{{item.sublotNumber}}</div>
            <div class="line">{{item.level}}</div>
        ...
        <div class="buttom-box">
          <div class="line3">{{item.lotNumber}}</div>
        </div>
      </li>
    </ul>
  </div>
...
//          console.log(qrcodeDoms)
        for (let i = 0; i < this.printData.length; i++) {
          ...
          let qrcode = new QRCode(qrcodeDoms[i], {
            text: this.printData[i].lotNumber,
            width: 280,
            height: 280
          })
          console.log(qrcode)
