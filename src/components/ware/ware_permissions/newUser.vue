  ...
  <div class="new">
    <el-dialog title="新增用户" :visible.sync="dialogVisible" width="40%">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="resetForm('registeredForm')">重置</el-button>
        <el-button type="primary" @click="submitForm('registeredForm')">立即创建</el-button>
      </span>
    </el-dialog>
  </div>
</template>
    ...
    submitForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$api.addwareUser(this.registeredForm).then(res => {
            console.log(res)
